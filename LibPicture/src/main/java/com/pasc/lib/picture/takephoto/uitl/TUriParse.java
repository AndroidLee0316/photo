...
public class TUriParse {
    ...
    public static File getFileWithUri(Uri uri, Activity activity) {
        ...
        if (ContentResolver.SCHEME_CONTENT.equals(scheme)) {
            ...
            if(cursor != null){
                ...
                if(isQQMediaDocument(uri.getPath())){
                    File fileDir = Environment.getExternalStorageDirectory();
                    File file = new File(fileDir, uri.getPath().substring("/QQBrowser".length(), uri.getPath().length()));
    ...
    private static boolean isQQMediaDocument(String path) {
        return path.contains("/QQBrowser");
    }
