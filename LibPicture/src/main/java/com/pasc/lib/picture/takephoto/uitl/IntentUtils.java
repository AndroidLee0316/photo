package com.pasc.lib.picture.takephoto.uitl;
...
public class IntentUtils {
     ...
     */
    public static Intent getPickMultipleIntent(TContextWrap contextWrap, Class<? extends Activity> customPickActivity, int limit) {
...
//        intent.putExtra(Constants.INTENT_EXTRA_LIMIT, limit > 0 ? limit : 1);
        ...
        Class<? extends Activity> activityClass=PictureSelectActivity.class;
        if (customPickActivity!=null){
            activityClass=customPickActivity;
        }
        Intent intent = new Intent (contextWrap.getActivity (), activityClass);
        intent.putExtra (TConstant.INTENT_EXTRA_LIMIT, limit > 0 ? limit : 1);
        return intent;
    }
